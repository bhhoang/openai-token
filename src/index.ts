...
const HttpCookieAgent = createCookieAgent(httpProxy.HttpProxyAgent)
const HttpsCookieAgent = createCookieAgent(httpsProxy.HttpsProxyAgent)
...
class Authenticator {
    ...
    private proxy?: {
        ...
        httpsAgent: CookieAgent<http.Agent>
    } | null = null

    constructor(private email: string, private password: string, private proxyServer: string = '') {
        this.jar = new CookieJar()
        if (proxyServer && proxyServer.includes(':')) {
            const [ host, port ]: string[] = proxyServer.split(':')
            this.proxy = {
                httpAgent: new HttpCookieAgent({ cookies: { jar: this.jar }, host, port }),
                httpsAgent: new HttpsCookieAgent({ cookies: { jar: this.jar }, host, port })
            }
        }
        const options = this.proxy??{ jar: this.jar }
        this.client = wrapper(axios.create({ ...options }))
    }
